...
func checkCode(bodyString string, code string, user *discordgo.User, guild string, channel string, diff time.Duration) {
	...
	if strings.Contains(bodyString, "redeemed") {
		if settings.Nitro.Delay {
			logWithTime("<red>[-] " + response.Message + "</> Delay: " + strconv.FormatInt(int64(diff/time.Millisecond), 10) + "ms")
		} else {
			logWithTime("<red>[-] " + response.Message + "</>")
		}
		...
		if settings.Nitro.Delay {
			logWithTime("<yellow>[x] " + response.Message + "</> Delay: " + strconv.FormatInt(int64(diff/time.Millisecond), 10) + "ms")
		} else {
			logWithTime("<yellow>[x] " + response.Message + "</>")
		}
	} else {
...
func checkGiftLink(s *discordgo.Session, m *discordgo.MessageCreate, link string, start time.Time) {
	...
	if len(code[2]) < 16 {
		logWithTime("<red>[X] Auto-detected a fake code: " + code[2] + " from " + m.Author.String() + "</>")
		return
	}

	_, found := cache.Get(code[2])
	if found {
		logWithTime("<blue>[X] Auto-detected a duplicate code: " + code[2] + " from " + m.Author.String() + "</>")
		return
	}
	...
	logWithTime("<green>[-] " + s.State.User.Username + " sniped code: </><cyan>" + code[2] + "</> [/] <blue> From Server: </>" + guild.Name + " > " + channel.Name + "]")
